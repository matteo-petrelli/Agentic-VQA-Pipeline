...
def build_react_system_prompt():
    ...
    return (
        ...
        "Your task is to carefully analyze documents and provide accurate answers.\n\n"
        "## AVAILABLE TOOLS\n"
        ...
        "2. ocr_extract: Extract all text content from the document with layout structure.\n"
        "   Use this when you need precise text that is hard to read visually.\n\n"
        "3. entity_tag: Tag the extracted text with semantic entities (dates, names, numbers, etc.).\n"
        "   Use this AFTER ocr_extract when you need to identify and match specific entities.\n\n"
        ...
        "Confidence: [High / Medium / Low]\n\n"
        "## DECISION CRITERIA\n"
        "Apply these rules IN ORDER when deciding your answer:\n\n"
        "1. **ANSWER if possible**: If the document contains information that reasonably answers "
        "the question, even partially, provide that answer. Prefer answering over refusing.\n"
        "2. **Say 'Unable to determine' ONLY when**: there is a CLEAR, SPECIFIC contradiction "
        "between what the question asks and what the document shows (e.g., the question references "
        "an entity, date, or value that is demonstrably different or absent in the document after "
        "thorough analysis with multiple tools).\n"
        "3. **Do NOT say 'Unable to determine' just because**:\n"
        "   - The OCR text is imperfect or partially illegible\n"
        "   - You are not 100% certain (use Medium confidence instead)\n"
        "   - You found the information in a different section than expected\n\n"
        "## CONFIDENCE LEVELS\n"
        "- High: The evidence from the document clearly and directly supports the answer.\n"
        "- Medium: Evidence exists but there is some ambiguity or partial match.\n"
        "- Low: Evidence is weak or there is a possible contradiction.\n\n"
        "## RULES\n"
        "- Always start with visual_inspect to understand the document structure.\n"
        "- You can call each tool AT MOST ONCE.\n"
        "- After each tool call, you receive an Observation with the tool's output.\n"
        "- Use at least 2 tools before calling final_answer to ensure thorough analysis.\n"
